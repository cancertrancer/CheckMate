...
const validateCred = array => {
  ...
  for (i = 0; i < array.length; i++) {
      if (i % 2 !== 0) {
        const newValue = array[i] * 2;
        if (newValue > 9) {
          const minusNine = newValue - 9;
          newValues.push(minusNine);
        } else {
  ...
  const sum = newValues.reduce(reducer);
  if (sum % 10 === 0) {
    return true; 
  } else {
    return false; 
...
const invalidCardCompanies = nestedArr => {  
 nestedArr.forEach(arr => {
   if (arr[0] === 3) {
     if (companiesArray.indexOf('Amex') === -1) {
     ...
     console.log(`Company issuing card # ${arr} not found`);
   }
 });
}
